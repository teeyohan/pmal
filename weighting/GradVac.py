        ...
        for tn_i in range(self.task_num):
            ...
            for tn_j in task_index:
                for k in range(len(self.k_idx)):
                    ...
                    rho_ijk = torch.dot(pc_grads[tn_i,beg:end], grads[tn_j,beg:end]) / (pc_grads[tn_i,beg:end].norm()*grads[tn_j,beg:end].norm()+1e-8) # Algorithm 1 Line 12
                    if rho_ijk < self.rho_T[tn_i, tn_j, k]:
                        ...
                        pc_grads[tn_i,beg:end] += grads[tn_j,beg:end]*w # Algorithm 1 Line 14
                        # batch_weight[tn_j] += w.item()
                    self.rho_T[tn_i,tn_j,k] = (1-beta)*self.rho_T[tn_i,tn_j,k] + beta*rho_ijk # Algorithm 1 Line 16
